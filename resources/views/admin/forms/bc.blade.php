...
<p style="text-align:center; font-size:22px;   margin-bottom:25px;"><u>TO WHOMSOEVER IT MAY CONCERN</u></p>
    <p style="text-align:justify; line-height:200%; font-size:20px;   margin-bottom:50px;">
        ...
        <span style="margin-left:50px;"></span>This is to certify that <span style="font-weight:bolder; text-transform:capitalize; text-decoration:underline">{{$certificate->name}}</span> S/O,D/o: <span style="font-weight:bolder; text-transform:capitalize; text-decoration:underline">{{$certificate->father_name}}; Mother name:{{$certificate->mother_name}}; R/O {{$certificate->address}} </span> is a bonafide student of our school. He/She is studying in Class <span style="font-weight:bolder; text-transform:capitalize;">{{$certificate->class}} (Session:{{$certificate->session}})</span> vide Admission no. <span style="font-weight:bolder; text-transform:capitalize; text-decoration:underline">{{$certificate->adm_no}}.</span> His/Her Date of birth is <span style="font-weight:bolder; text-transform:capitalize;">{{date('d-m-Y', strtotime($certificate->dob))}} 
